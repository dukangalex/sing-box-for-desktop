...
try {
    ...
    if ($null -eq $process) {
        throw "The AngelaBox service command did not start."
    }
    ...
    if ($processExitCode -ne 0 -and $diagnosticOutput.Length -eq 0) {
        $diagnosticOutput = "AngelaBox service $ServiceAction exited with code $processExitCode without diagnostic output."
    }
    ...
    if ($null -ne $nativeErrorCode) {
        $diagnosticOutput = "Failed to start AngelaBox service $ServiceAction (Win32 error $nativeErrorCode): $($exception.Message)"
    } else {
        $diagnosticOutput = "Failed to start AngelaBox service $ServiceAction`: $($exception.Message)"
    }
    [System.IO.File]::WriteAllText($OutputPath, $diagnosticOutput, [System.Text.Encoding]::Unicode)
